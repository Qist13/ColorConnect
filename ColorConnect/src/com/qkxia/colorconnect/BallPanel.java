...
public class BallPanel extends JPanel {
    ...
    //private static final Cursor customCursor = Toolkit.getDefaultToolkit().createCustomCursor(image, point, TOOL_TIP_TEXT_KEY);

    private ColorConnect game;
    ...
    public BallPanel(ColorConnect game) {
        this.game = game;
    ...
    public int getSwapDirection(int x, int y) {
        ...
        if (col < COLS && right <= left && right <= top && right <= bottom) {
            direction = DIRECTION_RIGHT;
        }

        // if not in first row and top edge is cloesest
        if (row > 0 && top <= left && top <= right && top <= bottom) {
            direction = DIRECTION_UP;
        }

        // if not in last row and bottom edge is closeest
        if (row < ROWS && bottom <= left && bottom <= right && bottom <= top) {
    ...
    private int markChainsAndGetPointsInRow(int row) {
        ...
        for (int col = 1; col < COLS; col++) {
            ...
            // reaches 3, also mark previous tow.
            if (count == 3) {
    ...
    private int markChainsAndGetPointsInCol(int col) {
        ...
        // loop through each row of the row
    ...
    public int calculatePoints(int count) {
        int points = 0;
        if (count == 3) {
            points = 10;
        } else if (count == 4) {
    ...
    public void removeMarkedChains() {
        // pause beofre clearing the marked chains
        pause(1000);
